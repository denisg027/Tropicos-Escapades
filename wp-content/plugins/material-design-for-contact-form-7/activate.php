...
class CF7_Material_Design_Update {
    ...
    public function update_version_switch() {
		// We are no-longer setting a default for this
		// If it already has a value, we will continue to show the switch
		// If not, the switch is never shown
		/*
    ...
    public function download_premium_notice() {
		global $cf7md_fs;
		if( false && $cf7md_fs->is_free_plan() && !get_option( 'cf7md_options[download_premium_notice_dismissed]' ) ) {
			?>
    ...
    public function dismiss_admin_notice() {
		...
		if( !in_array( $notice, array('cf7md_options[download_premium_notice_dismissed]', 'cf7md_options[upgraded_from_v1_notice_dismissed]') ) ) {
			echo 'Not allowed';
			wp_die();
		}
